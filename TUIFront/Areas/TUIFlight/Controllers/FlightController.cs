...
namespace TUIFront.Areas.TUIFlight.Controllers
{
    ...
    public class FlightController : BaseController
    {
        ...
        public ActionResult UpdateFlight(int id)
        {
            ...
            flighModel.SelectedDepCountryId = airportsList.First(elt => elt.Id == flighModel.DepartureAirportId).CountryId.ToString();
            flighModel.SelectedDestCountryId = airportsList.First(elt => elt.Id == flighModel.DestinationAirportId).CountryId.ToString();


            flighModel.DepartAirports = airportsList.Where(elt => elt.CountryId.ToString() == flighModel.SelectedDepCountryId).Select(x => new SelectListItem()
            {
                Text = string.Concat(x.AirportCode, " ", x.AirportName),
                Value = x.Id.ToString()
            }).ToList();

            flighModel.DestinationAirports = airportsList.Where(elt => elt.CountryId.ToString() == flighModel.SelectedDestCountryId).Select(x => new SelectListItem()
            {
                Text = string.Concat(x.AirportCode, " ", x.AirportName),
                Value = x.Id.ToString()
            }).ToList();
        ...
        public JsonResult GetAirportsByCountryId(int CountryId = 0)
        {
            List<AirportModel> airportsList = GetOrSetCacheData(CacheKey.Airports, () =>
              GetAirportsList());
            var result = airportsList.Where(elt => elt.CountryId == CountryId).ToList();
            return Json(result);
